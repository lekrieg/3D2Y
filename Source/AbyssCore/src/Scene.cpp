...
void abyss::Scene::Serialize(const std::string &path)
{
	...
	serializer::Serializer(&m_entityManager, &m_componentManager, &m_application->GetAssets()).Serialize(emitter);
	emitter << YAML::EndMap;
...
bool abyss::Scene::Deserialize(const std::string &path)
{
	...
	{
		serializer::Serializer(&m_entityManager, &m_componentManager, &m_application->GetAssets()).Deserialize(entities);
		return true;
	}
