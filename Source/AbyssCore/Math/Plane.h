...
namespace abyss
{
	namespace math
	{
		...
		public:
			...
			void CreatePlaneFromTri(Vector3D& t1, Vector3D& t2, Vector3D& t3);
			...
			bool ClipTriangle(Vector3D* inVerts, int totalInVerts, Vector3D* outFrontVerts, int* totalOutFrontVerts, Vector3D* outBackVerts, int* totalOutBackVerts);
