...
namespace abyss
{
	namespace math
	{
		...
		Matrix4x4 Matrix4x4::operator*(const Matrix4x4& m)
		{
			...
			return Matrix4x4(newMatrix[0], newMatrix[1], newMatrix[2], newMatrix[3],
		...
		void Matrix4x4::SetRotationRadians(double x, double y, double z)
		{
			...
			matrix[0] = static_cast<float>(cosY * cosZ);
			matrix[1] = static_cast<float>(cosY * sinZ);
			matrix[2] = static_cast<float>(-sinY);

			matrix[4] = static_cast<float>((sinX * sinY) * cosZ - cosX * sinZ);
			matrix[5] = static_cast<float>((sinX * sinY) * sinZ + cosX * cosZ);
			matrix[6] = static_cast<float>(sinX * cosY);

			matrix[8] = static_cast<float>((cosX * sinY) * cosZ + sinX * sinZ);
			matrix[9] = static_cast<float>((cosX * sinY) * sinZ - sinX * cosZ);
			matrix[10] = static_cast<float>(cosX * cosY);
		}

		void Matrix4x4::Rotate(float angle, int x, int y, int z)
		{
			angle = static_cast<float>(DEG_TO_RAD(angle));
			float cosAngle = static_cast<float>(cos(angle));
			float sineAngle = static_cast<float>(sin(angle));
		...
		void Matrix4x4::RotateAxis(double angle, Vector3D axis)
		{
			...
			float sinAngle = static_cast<float>(sin(PI_CONST * angle / 180));
			float cosAngle = static_cast<float>(cos(PI_CONST * angle / 180));
		...
		{
			matrix[5] = static_cast<float>(cos(PI_CONST * angle / 180));
			matrix[6] = static_cast<float>(sin(PI_CONST * angle / 180));
		...
		{
			matrix[0] = static_cast<float>(cos(PI_CONST * angle / 180));
			matrix[2] = -static_cast<float>(sin(PI_CONST * angle / 180));
		...
		{
			matrix[0] = static_cast<float>(cos(PI_CONST * angle / 180));
			matrix[1] = static_cast<float>(sin(PI_CONST * angle / 180));

			matrix[4] = -matrix[1];
			matrix[5] = matrix[0];
		}

		void Matrix4x4::CreateViewMatrix(Vector3D pos, Vector3D dir, Vector3D up, Vector3D right)
		{
