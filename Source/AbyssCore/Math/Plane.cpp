...
namespace abyss
{
	namespace math
	{
		...
		ABYSS_PLANE_STATUS Plane::ClassifyPoint(float x, float y, float z)
		{
								 ...
								 Vector3D *outBackVerts, int *totalOutBackVerts)
		{
			...
			ABYSS_PLANE_STATUS result = ABYSS_PLANE_STATUS::ABYSS_PLANE_FRONT;
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (result == ABYSS_PLANE_STATUS::ABYSS_PLANE_FRONT)
				{
					side[i] = static_cast<int>(ABYSS_PLANE_STATUS::ABYSS_PLANE_FRONT);
					numFront++;
				}
				if (result ==  ABYSS_PLANE_STATUS::ABYSS_PLANE_BACK)
				{
					side[i] = static_cast<int>(ABYSS_PLANE_STATUS::ABYSS_PLANE_BACK);
					numBack++;
				}
				else
				{
